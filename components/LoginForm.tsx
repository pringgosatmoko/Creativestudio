...
export const LoginForm: React.FC<LoginFormProps> = ({ onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isRegister) {
        ...
        setSuccessMsg('PENDAFTARAN BERHASIL: Akun sedang diproses. Silakan hubungi admin untuk aktivasi instan.');
      } else {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-[360px] mx-auto space-y-5 px-4 pb-10">
      <form onSubmit={handleSubmit} className="glass-panel p-8 rounded-[2.5rem] border-white/5 bg-slate-900/60 shadow-[0_0_50px_rgba(0,0,0,0.5)] space-y-6">
        <div className="flex justify-between items-center">
          <h2 className="text-[11px] font-black uppercase tracking-[0.4em] text-white">
            {isRegister ? 'Node Registration' : 'Initialize Access'}
          </h2>
          <div className={`w-2 h-2 rounded-full ${isRegister ? 'bg-fuchsia-500 shadow-[0_0_10px_#d946ef]' : 'bg-cyan-500 shadow-[0_0_10px_#22d3ee]'} animate-pulse`}></div>
        </div>

        <AnimatePresence mode="wait">
          {error && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="p-4 rounded-2xl bg-red-500/10 border border-red-500/20 text-red-400 text-[10px] text-center font-bold uppercase tracking-widest leading-relaxed flex flex-col gap-3">
              <span>{error}</span>
              {error.includes("Admin") && (
                <a 
                  href="https://t.me/pringgosatmoko" 
                  target="_blank" 
                  rel="noopener noreferrer" 
                  className="py-3 bg-white/5 border border-white/10 text-white rounded-xl flex items-center justify-center gap-2 hover:bg-white/10 transition-all text-[9px]"
                >
          ...
          {successMsg && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="p-5 rounded-2xl bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 text-[10px] text-center font-bold uppercase tracking-widest leading-relaxed flex flex-col gap-4">
              <i className="fa-solid fa-circle-check text-2xl mb-1"></i>
              <span>{successMsg}</span>
              <a 
                href="https://t.me/pringgosatmoko" 
                target="_blank" 
                rel="noopener noreferrer" 
                className="py-3.5 bg-cyan-500 text-black rounded-xl flex items-center justify-center gap-2 hover:bg-cyan-400 transition-all active:scale-95 shadow-[0_0_20px_rgba(34,211,238,0.3)] font-black"
              >
        ...
        {!successMsg && (
          <div className="space-y-4">
            {isRegister && (
              <>
                <div className="space-y-2">
                  ...
                  <input type="text" required value={fullName} onChange={(e) => setFullName(e.target.value)} className="w-full bg-black/40 border border-white/5 rounded-2xl py-4 px-6 text-white placeholder-slate-700 focus:outline-none focus:border-fuchsia-500/50 transition-all text-xs" placeholder="Identity Name" />
                </div>
                <div className="space-y-2">
            ...
            <div className="space-y-2">
              <label className="text-[9px] font-black uppercase tracking-widest text-slate-500 ml-1">Access Token</label>
        ...
        {successMsg && (
          <button 
            onClick={() => { setSuccessMsg(''); setIsRegister(false); }}
            className="w-full py-4 bg-white/5 border border-white/10 rounded-2xl text-[9px] font-black uppercase tracking-widest text-slate-500 hover:text-white transition-all"
          >
            Kembali ke Login
          </button>
        )}
      ...
      {!successMsg && (
        <div className="text-center space-y-4">
          <button onClick={() => { setIsRegister(!isRegister); setError(''); setSuccessMsg(''); }} className="text-[9px] font-black uppercase tracking-[0.4em] text-slate-600 hover:text-cyan-400 transition-colors">
            {isRegister ? '← Back to Authentication' : 'Request Access / Register'}
          </button>
          
          <div className="pt-2">
            <p className="text-[8px] font-black uppercase tracking-widest text-slate-700 mb-2">Registration Help?</p>
            <a href="https://t.me/pringgosatmoko" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-[9px] font-black uppercase tracking-widest text-cyan-500/40 hover:text-cyan-400 transition-all bg-white/5 px-4 py-2 rounded-full border border-white/5">
              <i className="fa-brands fa-telegram"></i>
              <span>Contact Master Pringgo</span>
            </a>
          </div>
        </div>
      )}
